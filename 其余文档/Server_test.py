...
async def generate_summary(
    ...
    custom_system_prompt: str = Body(None, description="自定义系统提示词（可选，不传则用默认规则）"),
    custom_user_prompt: str = Body(None, description="自定义用户提示词（可选，不传则用默认模板）")
):
    """
    基于会议转录文本生成结构化会议纪要
...
4. 确保语言通顺，无语法错误，信息无遗漏"""
        ...
        return {
            ...
            "used_system_prompt": final_system_prompt,  # 新增：返回实际使用的Prompt（便于调试）
            "used_user_prompt": final_user_prompt       # 新增：返回实际使用的Prompt（便于调试）
        }
